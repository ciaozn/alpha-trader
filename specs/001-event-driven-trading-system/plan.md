...
| 字段 | 值 |
|---|---|
| 状态 | Approved v1.0 · **执行中**（2026-09-12 进度审计：P0/P1/P2 已完成，P3 完成约 2/3） |
| 创建日期 | 2026-09-08 |
| 上游文档 | [spec.md](./spec.md)（Approved）· [docs/DESIGN.md](../../docs/DESIGN.md) |
| 开发方法 | SDD：本 plan 把 spec 的需求映射为有序的技术实施阶段 |
| 任务清单 | [tasks.md](./tasks.md)（P0-P3 细粒度任务 + 执行日志） |
...
| P3 | 风控 + OMS 全链路 | testnet 信号→风控→下单→成交 | RK-01~08、EX-01~06、OP-01/04 | 场景 2、场景 3 验收通过 | ⏳ 进行中（T301-T314 完成，T315-T320 待办，T321 阻塞） |
| P4 | 加固 + OKX + 告警 | 可 7×24 无人值守的模拟盘 | GW-03/06、OP-02/03、EN-05、RK-06/09、EX-05 | SC-03（7 天连续运行） | ⬜ 未开始 |
| P5 | 小资金实盘 | 实盘运行 | SEC-02、全部 P0 | SC-05、SC-06 | ⬜ 未开始 |
...
**已完成 16/23 项**（T301-T314 含两个补充任务），剩余 6 个待办 + 1 个阻塞（T321 需 testnet API Key）。

**当前能力边界（重要）**：风控与 OMS 的**逻辑与测试**已就绪，但**在线链路尚未通电**——paper/live 装配里还没有 SignalRecorder → RiskGate → OMS 这条序列，因此「场景 2 模拟盘全链路」与「场景 3 硬闸门端到端」都还不能验收。下一步按 T315 → T316 → T317 → T318 → T318b → T319 → T320 顺序推进，T319 是通电的那一步。
...
| 工作区有未提交 WIP（T315 三个文件已写未入库） | P3 | 恢复工作先 `git status` 确认，尽快提交或明确标注半成品 | 文件丢失则按 tasks.md 的验收标准重写（parser 是纯函数，重写成本低） |
...
1. **T315 精度对齐收口**：`BinanceExchangeInfoParser` / `BinanceTradingRulesFetcher`（代码已在磁盘、未提交）接进在线装配的 `TradingRulesProvider`，补脏单被拒测试
2. **T316 签名 REST**：HmacSHA256 + TimeSync 偏移 + 下单/撤单/账户/持仓/挂单，含边界 8 漂移守卫
3. **T317 user data stream**：listenKey 创建 + 30min keepalive + 回报解析 → `OrderReportEvent`
4. **T318 / T318b 对账与快照**：60s 对账（以交易所为准）+ 权益/持仓采样
5. **T319 在线装配通电**（关键）：`SignalRecorder` → `RiskGate` → OMS 按注册顺序接进 handler 序列，注意顺序约束（recorder 必须在 gate 之前）
6. **T320 场景 3 端到端离线验收**；**T321 场景 2** 需 testnet API Key（用户验收项，需要用户提供后执行）
7. P3 出口达成后进入 P4：邮件告警、REST 状态、崩溃恢复、OKX 网关、Docker 部署
