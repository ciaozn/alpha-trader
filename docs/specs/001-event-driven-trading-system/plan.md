...
| P4 | 加固 + OKX + 告警 | 可 7×24 无人值守的模拟盘 | GW-03/06、OP-02/03、EN-05、RK-06/09、EX-05 | SC-03（7 天连续运行） | ✅ 完成 T401-T408（含 T406 app 级网关选择、T407 镜像实测构建、T408 真实 MySQL 验证）；仅 SC-03 待用户跑 7 天 |
...
> **进度（2026-09-12，已完成）**：细任务见 [tasks.md](./tasks.md) §P4（T401-T409）。**T401-T408 全部完成**：
> 邮件告警、`/api/status`、崩溃恢复（日志回放校验）、风控热更新（二次确认）、幽灵仓位可选自动平仓、
> OKX 网关（含 `alpha.online.gateway=binance|okx` 选择开关）、Docker 部署、SQLite/MySQL 双方言。
> **T407 与 T408 都是「实测」而非「写了就算」**：T408 对真实 MySQL 首跑就抓出 `CREATE INDEX IF NOT EXISTS`
> 的方言 bug；T407 在本机 colima 里构建出镜像，并在容器内挂载真实 K 线跑通完整回测（报告落盘）。
> 剩余只有 **SC-03（testnet 连续 7 天）与 T321（场景 2 联调）**，两者都需要用户的 testnet 凭据。
